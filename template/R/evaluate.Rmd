...
sm.cv.sp <- krige.cv(formula = cbrt_depth ~ elev + slope, 
                       locations = dat_sp, model = model.geo, 
...
ggplot(results.cv, aes(x = depth, y = pred.btf)) +
  geom_point(aes(colour = modeltype, shape = modeltype), position = position_dodge(width = 2), size = 0.5, alpha = 0.3)+
  geom_abline(slope = 1) +
  coord_equal(ylim = c(-50, 500)) +

...
#test cropping by bounding box
  NP_boundaries <- readOGR(dsn = "../data/NP_boundaries/National_Parks__England____Natural_England.shp", 
                           verbose = TRUE) %>% st_as_sf()
  site_bound <- NP_boundaries[which(NP_boundaries$NAME == "YORKSHIRE DALES"),] %>% as_Spatial()
  # re-project
  site_bound <- spTransform(site_bound, CRSobj = proj4string(dat_sp))
  site_bbox <- st_bbox(c(xmin = 358152.2, ymin = 450897.0, 
                         xmax = 378152.2, ymax = 470897.0), crs = st_crs(elevation_st))#st_bbox(site_bound)
...
for(i in 1:nrow(tiles)) {
  ...
  elev_tile <- read_stars(parameters$elev_raster_fn)
  slope_tile <- read_stars(parameters$slope_raster_fn)
  
  elev_tile <- elev_tile[site_bbox]
  slope_tile <- slope_tile[site_bbox]
  
  preds_tile <- c(elev_tile, slope_tile)
  preds_tile <- read_stars(preds_tile, proxy = F)
  names(preds_tile) <- c("elev", "slope")
  
  st_dimensions(preds_tile)
  
  #read to local raster to assess if there are any non-null values
  slope_tile_r <- raster(slope_tile$slope_eng_5m.tif, crs = st_crs(slope_tile))
...
```{r}
plot(st_as_sfc(site_bbox), reset = FALSE, border = NA) #this effectively crops the plotting window without the hassle of cropping the data
plot(elevation_st, add = TRUE)
plot(st_as_sfc(site_bbox), add = TRUE, border = "red")

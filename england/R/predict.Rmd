...
```{r}
tracking <- cbind(i = as.integer(), xoff = as.numeric(), yoff = as.numeric(), Gb = as.character(), seconds = as.numeric()) %>% data.frame()
...
tiles <- data.frame(xoff = rep(seq(1, by = xsize, length.out = xnfull+1), ynfull+1), 
                    ...
                    yxize = c(rep(ysize, length.out = (xnfull+1)*ynfull), rep(yremainder, xnfull+1)))

for(i in 186){#196){    #nrow(tiles)) {
start <- Sys.time()
...
krige_out <- gstat::krige(formula = cbrt_depth ~ elev + slope, maxdist = 50, 
                          locations = sp_depth, 
                          newdata = preds_tile, 
                          model = model.geo)

...
# elev_tile_r <- raster(elev_tile$elev_eng_5m.tif, crs = st_crs(elev_tile))
# elev_tile_r[is.na(elev_tile_r)] <- 9999
# slope_tile_r <- raster(slope_tile$slope_eng_5m.tif, crs = st_crs(slope_tile))
# slope_tile_r[is.na(slope_tile_r)] <- 9999
# 
# elev_tile <- read_stars()

# elev_tile_sf <- st_as_sf(elev_tile)
# slope_tile_sf <- st_as_sf(slope_tile)
# 
# preds_tile_sf <- sf::st_join(elev_tile_sf, slope_tile_sf, left = F)
# names(preds_tile_sf) <- c("elev", "slope")
...
print(as.character(c(i = i, xoff = xoff, yoff = xoff, Gb = Gb)))
                       ...
                       maxdist = 2500, na.action = na.pass, 
                       locations = dat_sf, 
                       newdata = preds_tile, 
                       model = model.geo)
...
write_stars(krige.out["var1.pred"], paste0("../outputs/pred_tile", i, "_x", xoff, "_y", yoff, ".tif"))
write_stars(krige.out["var1.var"], paste0("../outputs/var_tile", i, "_x", xoff, "_y", yoff, ".tif"))
